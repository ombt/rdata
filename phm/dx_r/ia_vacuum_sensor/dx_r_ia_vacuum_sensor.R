                       ...
                       options)
{
    ...
    #
    meanadc_min <- as.numeric(params["I_VACUUM_MEANADC_MIN", 
                                     "PARAMETER_VALUE"])
    numreadings_min <- as.numeric(params["I_VACUUM_NUMREADINGS_MIN", 
                                         "PARAMETER_VALUE"])
    #
    # get flagged records
    #
    flagged_results <- 
        with(results, results[(mean_adc<=meanadc_min) &
                              ((num_readings)>=numreadings_min),])
    #
    if (nrow(flagged_results) > 0) {
        ...
        #
        flagged_results_mean_adc <- flagged_results
        flagged_results_mean_adc$CHART_DATA_VALUE <- flagged_results$mean_adc
        flagged_results_mean_adc$CHART_DATA_VALUE_TYPE <- "MEAN_ADC"
        #
        flagged_results_num_readings <- flagged_results
        flagged_results_num_readings$CHART_DATA_VALUE <- flagged_results$num_readings
        flagged_results_num_readings$CHART_DATA_VALUE_TYPE <- "NUM_READINGS"
        #
        flagged_results <- rbind(flagged_results,
                                 flagged_results_mean_adc,
                                 flagged_results_num_readings)
    }
    #
    ...
    if (nrow(not_flagged_results) > 0) {
        ...
        #
        if (options$chart) {
            not_flagged_results_mean_adc <- not_flagged_results
            not_flagged_results_mean_adc$CHART_DATA_VALUE <- not_flagged_results$mean_adc
            not_flagged_results_mean_adc$CHART_DATA_VALUE_TYPE <- "MEAN_ADC"
            #
            not_flagged_results_num_readings <- not_flagged_results
            not_flagged_results_num_readings$CHART_DATA_VALUE <- not_flagged_results$num_readings
            not_flagged_results_num_readings$CHART_DATA_VALUE_TYPE <- "NUM_READINGS"
            #
            not_flagged_results <- rbind(not_flagged_results,
                                         not_flagged_results_mean_adc,
                                         not_flagged_results_num_readings)
        }
    }
    #
...
#
write_results <- function(options, records)
{
    append <- FALSE
    col.names <- TRUE
    #
